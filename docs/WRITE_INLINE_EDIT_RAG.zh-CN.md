...
结论是：可以，但它应该被设计成“FIM 中间替换”，而不是“光标续写”。
...
也就是说，FIM 的“middle”不再是空光标，而是一个需要被重新生成的段落或选区。
...
- 主进程构造 FIM prompt，并把检索片段作为隐藏 Markdown comment 注入。
...
- `src/main/services/write-inline-edit-service.ts`：FIM 编辑服务和 RAG prompt 注入。
...
- `src/main/ipc/app-ipc-schemas.ts`、`src/preload/index.ts`、`src/shared/ds-gui-api.ts`：新增 `write:inline-edit` IPC。

测试覆盖：

- 编辑 payload schema。
- FIM 编辑请求和 replacement 提取。
- RAG 片段注入编辑 prompt。
...
这一版先把最关键的闭环打通：**选中词语 -> 段落级 FIM 编辑 -> BM25/关键词 RAG 辅助 -> 原地替换**。它和补全共享基础能力，但交互目标明确不同：补全是在光标后加字，编辑是在一个受控范围里重写。
